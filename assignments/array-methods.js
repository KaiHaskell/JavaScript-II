...
runnersLargeSizeShirt = runners.filter(items => items.shirt_size == "L");
...
ticketPriceTotal.push(
  runners.reduce((moneymoneymoney, items) => {
    return moneymoneymoney + items.donation;
  }, 0)
);
...
// Problem 1
// Everyone knows that companies with two oo's are destined to be succesful. Create an alphabetized list to help spot out such companies.

let companyNames = [];
runners.forEach(function(items) {
  return companyNames.push(`${items.company_name}`);
});
